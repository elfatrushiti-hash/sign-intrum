...
export default function ImpactScore({ data, t }) {
  ...
  const getColor = () => {
    if (score >= 70) return "bg-intrumTeal";
    if (score >= 40) return "bg-intrumLightBlue";
    return "bg-intrumViolet";
  };

  return (
    <div className={`p-6 rounded-xl shadow text-white ${getColor()}`}>
      <h4 className="text-xl font-bold mb-2">{t.impactScore.title}</h4>
      <p className="text-4xl font-extrabold">{animatedScore} / 100</p>
      <p className="mt-2 text-white/90 text-sm">{t.impactScore.subtitle}</p>
    </div>
  );
}
